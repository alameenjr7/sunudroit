    ...
    <div class="row mb-4">
        <div class="col-md-12 mb-4">
            <div class="card text-left">
                <div class="card-body">
                    <h4 class="card-title mb-3">Liste des Infos</h4>
                    <p class="float-right"> Total Infos Pratiques: {{$infos->count()}}</p>
                    <div class="col-lg-12">
                                ...
                                @foreach ($infos as $item)
                                    <tr>
                                        ...
                                        <td style="text-align: center">
                                            <div class="icon {{$item->icons}}">{{$item->icons}}</div>
                                        </td>
                                        <td>
                                            <input 
                                                type="checkbox" 
                                        ...
                                        <td>
                                            <a class="btn btn-success float-left" href="{{route('info_pratique.edit',$item->id)}}"
                                                title="Modifier" data-toggle="tooltip" data-placement="bottom">
                                                <i class="nav-icon i-Pen-2"></i>
                                            </a>
                                            <form action="{{route('info_pratique.destroy', $item->id)}}" method="post">
                                                @csrf
                                                @method('delete')
...
<script>
    $('input[name=toogle]').change(function(){
        ...
        // alert(id);
        $.ajax({
            url:"{{route('info.pratique.status')}}",
            type:"POST",
            data:{
                _token:'{{csrf_token()}}',
                mode:mode,
